...
from pyxdi._async import AsyncDIContext  # noqa
...
@pytest.fixture
def di_context() -> AsyncDIContext:
    return AsyncDIContext()


async def test_close(di_context: AsyncDIContext) -> None:
    events = []
    ...
    assert await di_context.get(str) == "test"

    await di_context.close()

    assert events == ["dep1:before", "dep1:after"]


async def test_bind_transient_scoped_generator_provider(
    di_context: AsyncDIContext,
) -> None:
    ident = "test"
    ...
    service = await di_context.get(Service)

    assert service.ident == "test"
    assert service.events == ["before", "after"]


async def test_bind_transient_scoped_async_generator_provider(
    di_context: AsyncDIContext,
) -> None:
    ident = "test"
...
async def test_bind_request_scoped_async_generator_provider(
    di_context: AsyncDIContext,
) -> None:
    ident = "test"
    ...
    async with di_context.request_context():
        service = await di_context.get(Service)
...
async def test_get_and_set_with_request_context(di_context: AsyncDIContext) -> None:
    @di_context.provide(scope="request")
    def service(ident: str) -> Service:
        return Service(ident=ident)

    async with di_context.request_context() as ctx:
        ctx.set(str, "test")

        assert (await di_context.get(Service)).ident == "test"


async def test_get_request_scoped_not_started(di_context: AsyncDIContext) -> None:
    ident = "test"
    ...
    with pytest.raises(LookupError) as exc_info:
        await di_context.get(Service)

    assert str(exc_info.value) == "Request context is not started."


async def test_get_provider_arguments(di_context: AsyncDIContext) -> None:
    @di_context.provide()
    def a() -> int:
        return 10
    ...
    assert (await di_context.get(Service)).ident == "10/1.0/test"


async def test_inject_callable_with_async_target(di_context: AsyncDIContext) -> None:
    @di_context.provide()
    async def ident() -> str:
        return "1000"
...
async def test_inject_callable_with_sync_target(di_context: AsyncDIContext) -> None:
    @di_context.provide()
    def ident() -> str:
        return "1000"
