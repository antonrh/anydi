...
class TestResolver:
    ...
    async def test_resolver_async_singleton_locking(self) -> None:
        """Test async singleton resolution with locking."""
        container = Container()

        call_count = 0

        @container.provider(scope="singleton")
        async def my_async_int() -> int:
            nonlocal call_count
            call_count += 1
            await asyncio.sleep(0.01)
            return 42

        result = await container.aresolve(int)
        assert result == 42
        assert call_count == 1

        # Second resolution should use cached value
        result2 = await container.aresolve(int)
        assert result2 == 42
        assert call_count == 1

    async def test_resolver_async_coroutine_with_params(self) -> None:
        """Test async coroutine provider with parameters."""
        container = Container()
        ...
        container.register(Config, lambda: Config(10), scope="singleton")

        @container.provider(scope="singleton")
        async def my_service(config: Config) -> Service:
            return Service(config)

        result = await container.aresolve(Service)
        assert result.config.value == 10

    async def test_resolver_async_generator_with_params(self) -> None:
        """Test async generator provider with parameters."""
        container = Container()
        ...
        container.register(Config, lambda: Config(20), scope="singleton")

        @container.provider(scope="request")
        async def my_resource(config: Config) -> AsyncIterator[Resource]:
        ...
        async with container.arequest_context():
            result = await container.aresolve(Resource)
            assert result.config.value == 20
            assert not result.closed

        assert result.closed

    def test_resolver_sync_generator_with_params(self) -> None:
        """Test sync generator provider with parameters."""
        container = Container()
        ...
        container.register(Config, lambda: Config(30), scope="singleton")

        @container.provider(scope="request")
        def my_resource(config: Config) -> Iterator[Resource]:
            resource = Resource(config)
            try:
                yield resource
            finally:
        ...
        with container.request_context():
            result = container.resolve(Resource)
            assert result.config.value == 30
            assert not result.closed

        assert result.closed

    async def test_resolver_async_from_context_provider(self) -> None:
        """Test async resolution of from_context provider."""
        container = Container()
        container.register(int, scope="request", from_context=True)
    ...
    async def test_resolver_async_from_context_with_override_test_mode(self) -> None:
        """Test async from_context provider with override in test mode."""
        container = Container()
        container.enable_test_mode()
        container.register(int, scope="request", from_context=True)
        ...
        async with container.arequest_context() as ctx:
            ctx.set(int, 50)
            with container.override(int, 200):
                result = await container.aresolve(int)
                assert result == 200
    ...
    async def test_resolver_async_class_context_manager(self) -> None:
        """Test async resolution of class implementing async context manager."""
        container = Container()
        ...
        class AsyncContextResource:
            async def __aenter__(self) -> "AsyncContextResource":
                nonlocal entered
                entered = True
                return self
        ...
        container.register(AsyncContextResource, scope="request")

        async with container.arequest_context():
            result = await container.aresolve(AsyncContextResource)
            assert isinstance(result, AsyncContextResource)
            assert entered

        assert exited

    async def test_resolver_async_sync_class_context_manager(self) -> None:
        """Test async resolution of class implementing sync context manager."""
        container = Container()

        entered = False
        exited = False

        class SyncContextResource:
            def __enter__(self) -> "SyncContextResource":
                nonlocal entered
                entered = True
                return self
        ...
        container.register(SyncContextResource, scope="request")

        async with container.arequest_context():
            result = await container.aresolve(SyncContextResource)
            assert isinstance(result, SyncContextResource)
            assert entered
