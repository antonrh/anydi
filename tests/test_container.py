...
class TestContainerCustomScopes:
    ...
    def test_scoped_context_isolated_keeps_parent_scope(
        self, container: Container
    ) -> None:
        """Test that an isolated context still resolves parent scope dependencies."""
        container.register_scope("task")
        container.register_scope("workflow", parents=["task"])
        container.register(UniqueId, scope="task")  # type: ignore[arg-type]
        container.register(Service, scope="workflow", from_context=True)

        with container.scoped_context("task"):
            task_instance = container.resolve(UniqueId)

            with container.scoped_context("workflow") as outer:
                outer.set(Service, Service(ident="outer"))

                with container.scoped_context("workflow", isolated=True) as inner:
                    # The parent scope is untouched
                    assert container.resolve(UniqueId) is task_instance

                    # Values set on the shadowed context are not inherited
                    with pytest.raises(LookupError):
                        container.resolve(Service)

                    inner.set(Service, Service(ident="inner"))

                    assert container.resolve(Service).ident == "inner"

                assert container.resolve(Service).ident == "outer"
