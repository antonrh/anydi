...
def compile_resolver(  # noqa: C901
    ...
    *,
    ...
    if is_async:
        create_lines.append(
            "async def _create_instance(container, context, store_result, defaults):"
        )
    else:
        create_lines.append(
    ...
    if no_params:
        ...
        if not is_async:
            ...
            create_lines.append(
                "        raise TypeError("
                'f"The instance for the provider `{_provider_name}` cannot be created '
                'in synchronous mode."'
                ")"
            )
    else:
        ...
        if is_async:
            create_lines.append("    resolver_cache = container._async_resolver_cache")
        else:
    ...
    if not no_params:
        ...
        for idx, name in enumerate(param_names):
            ...
            create_lines.append(
                f"            if _param_annotations[{idx}] in _unresolved_interfaces:"
            )
            create_lines.append(
                f"                raise LookupError(_unresolved_messages[{idx}])"
            )
            create_lines.append("            try:")
            create_lines.append(f"                subprov = _param_providers[{idx}]")
            ...
            if wrap_dependencies:
                create_lines.append(
                    f"    arg_{idx} = container._wrap_compiled_dependency("
                    f"_provider, _param_annotations[{idx}], arg_{idx})"
                )
    ...
    if is_async and is_coroutine:
        ...
        if param_names:
            ...
            create_lines.append("        inst = await _provider_call(**call_kwargs)")
            create_lines.append("    else:")
        ...
        if param_names:
            ...
            create_lines.append("    else:")
            create_lines.append("        cm = _asynccontextmanager(_provider_call)()")
    ...
    if wrap_instance:
        create_lines.append(
            "    inst = container._after_compiled_resolve(_provider, inst)"
        )
    ...
    if has_override_support:
        resolver_lines.append("    override = container._get_override_for(_interface)")
    ...
    if scope == "singleton":
        ...
        if wrap_instance:
            resolver_lines.append(
                "        return container._after_compiled_resolve(_provider, inst)"
            )
        else:
    ...
    if is_async:
        ...
        create_resolver_lines.append("def _resolver_create(container, defaults=None):")
    ...
    if has_override_support:
        create_resolver_lines.append(
            "    override = container._get_override_for(_interface)"
        )
    ...
    if scope == "singleton":
        if is_async:
            create_resolver_lines.append(
                "    return await _create_instance(container, context, False, defaults)"
            )
        else:
            create_resolver_lines.append(
        ...
        if is_async:
            create_resolver_lines.append(
                "    return await _create_instance(container, None, False, defaults)"
            )
        else:
            create_resolver_lines.append(
    ...
    ns: dict[str, Any] = {
        ...
        "_unresolved_interfaces": unresolved_interfaces,
        "_NOT_SET": NOT_SET,
